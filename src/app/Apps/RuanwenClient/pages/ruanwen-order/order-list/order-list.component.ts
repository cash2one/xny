...
@Component({
  ...
  styleUrls: ['../../../../../Public/theme/apps-common/common.scss','../../../../../Public/theme/apps-common/table.scss','../../../ruanwen.component.scss','./order-list.component.scss']
})
export class OrderListComponent implements OnInit {
  ...
  public getOrderListData(page:number = 1):void{
    this.loadingType = 'show'
  	this.grOrderService.postOrderList({
      ...this.orderRetrieval,
      'page':page
    }).subscribe(res=>{
      this.loadingType = 'hide'
        ...
        else this.orderItem.map(e=>e['is_refres']=false)
  ...
  public fnOrderInfo(data:any):void{
    ...
    this.ruanwenReleaseService.handleOrderStatusRouter(data['status'],data['id'])
  ...
  public fnEditOrder(data:any):void{
    ...
    this.router.navigateByUrl("RuanwenClient/article/release/delivery/"+data['id'])
  ...
  public handleDelete(data:any):void{
   this.grOrderService.getOrderDel({
      'id':data['id']
    }).subscribe(res=>{
      if(res.status===1){
