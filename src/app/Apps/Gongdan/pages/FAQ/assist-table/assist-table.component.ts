...
@Component({
  ...
  styleUrls: ['../../../gongdan.component.scss','../../../../../Public/theme/common/common.scss','./assist-table.component.scss']
})
export class AssistTableComponent implements OnInit {
  ...
  public getHelpDocList( id : number, page : number | string = 1 ) : void{
    ...
    this.xnFaqServices.getProblemList({
      'cat_id': id,
      'name': '',
      'page': page
    }).subscribe(res=>{
  ...
  public sortListorder( data: any ) : void {
    ...
    if(this.autoSort != data['listorder']) {
      ...
      this.xnFaqServices.postProblemSort({
        'id': data['id'],
        'sort': data['listorder']
      }).subscribe(res=>{
        if(res.status===1){
