...
export type pboxOtherType = 'CLASSIFY_PARENT' | 'HELP_DOC' | ''
...
export class AssistMainComponent implements OnInit {
  ...
  public helpDocList : Array<any> // 帮助文档的列表
  ...
  获取帮助文档的函数
   */
  public getHelpDocList( name: string = '' ) : void {
    this.xnFaqServices.postProblemHelpList({
      'name': name
    }).subscribe(res=> {
      if(res.status===1) {
        this.assistMainService.setHelpDocData( res['data']['data'] )
  ...
  添加问题的时候选择添加帮助文档的函数
   */
  public acceptHelpDocData( data: any ) : void {
      this._pboxOtherType = 'HELP_DOC'
      this.handleOpenPbox( 'HELP_DOC' , data )
  }
  ...
  public handleDelClassify( id:number | string ) : void {
    ...
    this.xnFaqServices.postProblemcateDel({
      'id': id
    }).subscribe(res=>{
  ...
  public handleDelDoc( id : number ) : void {
    ...
    this.xnFaqServices.postProblemDel({
      'id': id
    }).subscribe(res=>{
  ...
  接受pbox搜索的数据
   */
  public acceptSearchPbox( search: string ): void {
    if(this._pboxOtherType == 'HELP_DOC') this.getHelpDocList(search)
  }
