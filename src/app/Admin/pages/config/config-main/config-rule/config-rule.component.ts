...
import { ConfigRuleData, AddOrEditList } from './configRuleData'

import { animations } from '../../../../../Public/Animations/index'

@Component({
  selector: 'app-config-rule',
  templateUrl: './config-rule.component.html',
  styleUrls: [
    '../../../../Admin.component.scss',
    // './config-rule.component.scss',
    '../../../page.common.scss',
    './config-rule.component.new.scss'
  ],
  animations: [
    animations.flyTop
  ]
})
export class ConfigRuleComponent implements OnInit,OnDestroy {
  ...
  constructor(
    ...
    this.AllCheck = false
    this.TitleList = new ConfigRuleData().titleData
    this.FlyLeftData = new ConfigRuleData().FlyLeftData
    this.ruleData = new ConfigRuleData().templateRuleData
    this.addOrEditList = new ConfigRuleData().addOrEditList
    this.modalShowIsAddOrEdit = new ConfigRuleData().addOrEditFlag
    this.showTplList = new ConfigRuleData().showDetailData
    this.showTemData = new ConfigRuleData().showTemDetailData
    this.bindTempTitle = new ConfigRuleData().bindTempTitle
    this.isLoading = true
    this.ShowConfirm = {
      isShow: false,
      menu_id: '',
  ...
  ngOnInit() {
    ...
    this.pboxObj =  this.riccioPboxService.getEmit().subscribe(v => {
      if(v.type === 'smsRuleDel'){
        this.opListBatch(v.data.ids,v.data.opType)
      }else if(v.type === 'smsRuleDelOne'){
        this.delRule(v.data)
      }
    })
    //监听模态关闭
  ...
  public fnGetRuleList(): void {
    this.isLoading = true
    this.grConfigService.getSmsRuleList(this.pageRequestParam)
      .subscribe(res => {
  ...
  public getAllTemp() {
    ...
    this.grConfigService.getSmsTplList().subscribe(res => {
      if (res.status === 1) {
        this.riccioLoadingService.closeLoading()
        this.bindTempList = res.data.data
  ...
  public fnRuleBind(temp: any) {
    ...
    this.grConfigService.postSmsRuleBind({
  ...
  public fnEnableRule(list: any, e: MouseEvent) {
    ...
    this.grConfigService.postSmsRuleStatus({
      id: list.id,
      status: status
    }).subscribe(res => {
      if (res.status === 1) {
  ...
  public fnSeeTemp(temId: number, e: MouseEvent) {
    ...
    this.grConfigService.postSmsRuleTplInfo({
      id: temId
    }).subscribe(res => {
      this.riccioLoadingService.closeLoading()
      if (res.status === 1) {
        this.showTemData.forEach((v, i, src) => {
          if (v['flag'] === "status") {
            src[i].value = (res['data'][src[i].flag] == 1 ? '审核中' : res['data'][src[i].flag] == 2 ? '已通过' : '未通过')
          } else {
  ...
  public delRule(id:number){
    ...
    this.grConfigService.postSmsRuleDel({
      id:id
    }).subscribe(res=>{
      this.riccioLoadingService.closeLoading()
      if(res.status === 1){
  ...
  public FnShowAddTemplate(): void {
    ...
    this.personalService.showViewData('config-rule-add', true)
  ...
  public FnSwitch(list: any): void {
    ...
    this.grConfigService.postSmsRuleEdit(list)
      .subscribe(res => {
        if (res.status === 1) {
          this.personalService.showPromptSmall('修改成功', 'success', { 'right': '50%', 'top': '30%' })
        } else if (res.status === 0) {
          this.personalService.showPromptSmall('修改失败', 'danger', { 'right': '50%', 'top': '30%' })
          list.status = list.status == 0 ? 1 : 0;
        }
      }, error => {
        console.error(error)
  ...
  public opListBatch(ids: Array<any>, type: number): void {
    ...
    for (let item in ids) {
      switch (type) {
        case 1:
          opBatch.push(this.grConfigService.postSmsRuleDel({ id: ids[item] }))
          break
        case 2:
          opBatch.push(this.grConfigService.postSmsRuleStatus({
            id: ids[item],
            status: 0
          }))
          break
  ...
  public FnDeleteMenu(): void {
    this.grConfigService.postSmsRuleDel({ id: this.ShowConfirm.menu_id })
      .subscribe(res => {
        this.ShowConfirm.isShow = !this.ShowConfirm.isShow;
        if (res.status === 1) {
          this.fnGetRuleList()
  ...
  //点击弹出绑定模板的选择事件
  public FnShowBindModel(list: any): void {
    this.personalService.showViewData('config-rule-bind', true)
  }
