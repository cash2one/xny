...
@Component({
  ...
  styleUrls: [
    '../../role.common.scss',
    './role-add-or-edit.component.scss'
  ]
})
export class RoleAddOrEditComponent implements OnInit {
  ...
  ngOnInit() {
    this.postRoleData.model = this.routerInfo.model
    this.postRoleData.cid = this.routerInfo.cid
  }
  ...
  public fnEditChange():void{
    ...
    this.grRoleService.getRoleInfo(data).subscribe(res=>{
      ...
      if(res.status===1){
        ...
        this.routerInfo.model !== 'Admin'?this.parentName = res['data']['parent_name']==""?'企业负责人':res['data']['parent_name']:{}
      }
    })
  }
  ...
  public fnAddEdit(emit:boolean):void{
    if(this.postRoleData['groupname'].trim()==""){
      ...
      this.grRoleService.postRoleAdd(this.postRoleData).subscribe(res=>{
        this.disabledBtnOpen = true
        if(res.status===1){
          this.closeView()
      ...
      this.disabledBtnOpen = false
      this.grRoleService.postRoleEdit({
        'id':this.roleInfoId,
        ...this.postRoleData
      }).subscribe(res=>{
        this.disabledBtnOpen = true
